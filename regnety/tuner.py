...
def make_model(hp, strategy=tpu_strat):
    hp_weight_decay = hp.Choice("weight_decay", [5e-5, 1e-4, 5e-4, 1e-3, 5e-3])
    hp_base_lr = hp.Choice("base_lr", [5e-4, 1e-3, 5e-3])
...
wandb.init(entity="compyle", project="regnety", job_type="train", name="Tuner200MF")
...
tuner.search(
    train_ds, epochs=75, validation_data=val_ds, callbacks=[stop_early, WandbCallback()]
)
now = datetime.now()
...
with tf.io.gfile.GFile(
    os.path.join("gs://ak-europe-train/tunercache/", "bestHP_%s.json" % date_time), "a+"
) as f:
