...
parser.add_argument(
    ...
    "--tpu_address",
    type=str,
    help="Network address of TPU clsuter",
    default=None,
)
parser.add_argument(
    "-tfp",
...
train_prep_cfg = get_preprocessing_config(
    tfrecs_filepath=train_tfrecs_filepath, augment_fn="default", mixup=True
)

val_prep_cfg = get_preprocessing_config(
    tfrecs_filepath=val_tfrecs_filepath, augment_fn="val", mixup=False
)
...
wandb.init(
    ...
    project="regnety",
    job_type="train",
    name="Final_" + date_time + "_" + flops.upper(),
)

trial_callbacks = [
    tf.keras.callbacks.LearningRateScheduler(tutil.get_train_schedule(train_cfg)),
    tf.keras.callbacks.TensorBoard(
        log_dir=os.path.join(train_cfg.log_dir, str(date_time)), histogram_freq=1
...
history = model.fit(
    train_ds, epochs=train_cfg.total_epochs, validation_data=val_ds, callbacks=callbacks
)
